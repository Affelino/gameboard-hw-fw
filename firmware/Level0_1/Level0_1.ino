//////////////////////////////////////////////////////////////////////
// Firmware: Level 0.1 Implementation for DIY Chessboard
//
...
uint8_t		moveFromRank = EMPTY, moveFromFile = EMPTY;
uint8_t		moveToRank = EMPTY, moveToFile = EMPTY;
uint8_t		returnToRank = EMPTY, returnToFile = EMPTY;
uint8_t		returnFromRank = EMPTY, returnFromFile = EMPTY;
...
void inTo( uint8_t rank, uint8_t file ) {
	...
	// If we are adding a piece on the board at startup or maybe for testing, just return.
	if( ( ( moveFromRank == EMPTY ) && ( moveFromFile == EMPTY ) ) ) {
		return;
	}
  
	// If we are returning piece to it's original position because it was took from a wrong place, just reset the indicator light.
	if( ( returnToRank == rank ) && ( returnToFile == file ) ) {
		board_g[rank][file].ledInfo = 0;
		returnToRank = EMPTY;
		returnToFile = EMPTY;
		return;
	}

	// If this is our turn, send the location.
	if( ( moveToRank == EMPTY ) && ( moveToFile == EMPTY ) ) {
		moveFromRank = EMPTY;
		moveFromFile = EMPTY;
	...
	// In case this was opponents move, make sure it is into right location.
	else if( ( moveToRank == rank ) && ( moveToFile == file ) ) {
		board_g[rank][file].ledInfo = 0;
		moveToRank = EMPTY;
		moveFromRank = EMPTY;
		moveToFile = EMPTY;
		...
		returnFromRank = rank;
		returnFromFile = file;
		ledColor = ( ( rank + file ) % 2 ) ? WARNING_WHITE_COLOR : WARNING_BLACK_COLOR;
		board_g[rank][file].ledInfo = WARNING_STATE_BLINK | ledColor;
		return;
	}
}
...
void outFrom( uint8_t rank, uint8_t file ) {
	...
	// Are we making our own move?
	if( ( moveToRank == EMPTY ) && ( moveToFile == EMPTY ) ) {
		// If we don't already have the "from" square defined, just update it.
		if( ( moveFromRank == EMPTY ) && ( moveFromFile == EMPTY ) ) {
			moveFromRank = rank;
			moveFromFile = file;
			...
			// If we had already picked up a piece, this one is probably removing the opponents piece that was captured
			// Don't report, just ignore.
			return;
		}
	}
	// If this wasn't our move then make sure we're moving the right piece for opponent
	else if( ( moveFromRank == rank ) && ( moveFromFile == file ) ) {
		board_g[rank][file].ledInfo = 0;
	}
	// If we are trying to reverse an illegal move, clear the indicator
	else if( ( returnToRank != EMPTY ) && ( returnToFile != EMPTY ) ) {
		board_g[rank][file].ledInfo = 0;
		return;
	}
	// If we are trying to return an illegal move, clear the indicator
	else if( ( returnFromRank == rank ) && ( returnFromFile == file ) ) {
		board_g[rank][file].ledInfo = 0;
		returnFromRank = EMPTY;
		returnFromFile = EMPTY;
		return;
	}
	// If the opponent captured our piece, don't inform about the remove
	else if( ( moveToRank == rank ) && ( moveToFile == file ) ) {
		return;
	}
	// If this seems like it's going to be an illegal move, inform by fast red blink and save the location for controlled return...
	else {
		ledColor = ( ( rank + file ) % 2 ) ? WARNING_WHITE_COLOR : WARNING_BLACK_COLOR;
		board_g[rank][file].ledInfo = WARNING_STATE_BLINK | ledColor;
		returnToRank = rank;
	...
	if ( receivedMessageLen == 4 ) {
		...
		switch ( receivedMessage[0] ) {
			...
			case PICK_MESSAGE:
				...
				moveFromRank = moveRank;
				moveFromFile = moveFile;
				moveToRank = WAIT;  // Special meaning, waiting for to-rank or remove message.
				moveToFile = WAIT;  // Special meaning, waiting for to-rank or remove message.
				break;
